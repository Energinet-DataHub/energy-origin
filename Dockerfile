...
RUN rm -f appsettings.json appsettings.*.json || true && \
    ...
    dotnet-subset restore --root-directory /src/${PROJECT} --output-directory /src/${PROJECT}/restore-subset
...
RUN dotnet restore ${PROJECT}/restore-subset/${PROJECT}.csproj && \
    ...
    dotnet publish -c Release -o /app/publish --no-restore --no-build
