...
public class OidcControllerTests
{
    ...
    public async Task CallbackAsync_ShouldReturnRedirectToFrontendWithError_WhenDiscoveryFails()
    {
        ...
        var result = await new OidcController().CallbackAsync(accessor, cache, factory, mapper, service, issuer, oidcOptions, tokenOptions, providerOptions, logger, Guid.NewGuid().ToString(), null, null);
    ...
    public async Task CallbackAsync_ShouldLogError_WhenDiscoveryFails()
    {
        ...
        _ = await new OidcController().CallbackAsync(accessor, cache, factory, mapper, service, issuer, oidcOptions, tokenOptions, providerOptions, logger, Guid.NewGuid().ToString(), null, null);
    ...
    public async Task CallbackAsync_ShouldReturnRedirectToFrontendWithErrorAndLogWarning_WhenGivenErrorConditions(string? error, string? errorDescription, string expected)
    {
        ...
        var result = await new OidcController().CallbackAsync(accessor, cache, factory, mapper, service, issuer, oidcOptions, tokenOptions, providerOptions, logger, null, error, errorDescription);
