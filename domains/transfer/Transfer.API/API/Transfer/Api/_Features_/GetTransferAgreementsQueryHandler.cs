...
public class GetTransferAgreementsQueryHandler(IUnitOfWork unitOfWork, TransferAgreementStatusService transferAgreementStatusService)
    : IRequestHandler<GetTransferAgreementsQuery, GetTransferAgreementQueryResult>
{
    public async Task<GetTransferAgreementQueryResult> Handle(GetTransferAgreementsQuery request, CancellationToken cancellationToken)
    {
        ...
        var transferAgreementProposals =
            await unitOfWork.TransferAgreementRepo.GetTransferAgreementProposals(request.OrganizationId, cancellationToken);
