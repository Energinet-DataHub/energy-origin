...
public class Api
{
    ...
    public async Task<IList<ActivityLogEntryResponse>> GetActivityLog(ActivityLogEntryFilterRequest request)
    {
        return await GetActivityLog(authenticatedClient, request);
    }

    public async Task<IList<ActivityLogEntryResponse>> GetActivityLog(string tin, ActivityLogEntryFilterRequest request)
    {
        return await GetActivityLog(factory.CreateAuthenticatedClient(sub, apiVersion: apiVersion, tin: tin), request);
    }

    public async Task<IList<ActivityLogEntryResponse>> GetActivityLog(HttpClient client, ActivityLogEntryFilterRequest request)
    {
        var result = await client.PostAsJsonAsync("/activity-log", request);
        output.WriteLine(await result.Content.ReadAsStringAsync());
        result.StatusCode.Should().Be(HttpStatusCode.OK);
        var activityLogResponseBody = await result.Content.ReadAsStringAsync();
        var log = JsonConvert.DeserializeObject<IList<ActivityLogEntryResponse>>(activityLogResponseBody);
        return log!;
    }
