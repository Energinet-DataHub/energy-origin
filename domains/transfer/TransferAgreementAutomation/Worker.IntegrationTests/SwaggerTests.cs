...
public class SwaggerTests : IClassFixture<TransferAutomationApplicationFactory>
{
    ...
    public SwaggerTests(TransferAutomationApplicationFactory factory)
    {
    ...
    public async Task GetSwaggerDocs_AllVersions_ReturnsOk()
    {
        ...
        {
            var swaggerDocResponse = await client.GetAsync($"api-docs/transfer-automation/{version}/swagger.json");
    ...
    public async Task SwaggerDocs_AllVersions_MatchSnapshots()
    {
        ...
        foreach (var version in provider.ApiVersionDescriptions.Select(v => v.GroupName))
        {
            var swaggerDocUrl = $"api-docs/transfer-automation/{version}/swagger.json";
