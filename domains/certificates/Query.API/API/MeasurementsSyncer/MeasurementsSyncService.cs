...
public class MeasurementsSyncService
{
    ...
    public MeasurementsSyncService(ILogger<MeasurementsSyncService> logger, ISlidingWindowState slidingWindowState,
        ...
        IMeasurementSyncMetrics measurementSyncMetrics, IMeasurementSyncPublisher measurementSyncPublisher, Meteringpoint.V1.Meteringpoint.MeteringpointClient meteringPointsClient, IOptions<MeasurementsSyncOptions> options)
    {
        _logger = logger;
        ...
        CancellationToken stoppingToken)
    {
        ...
        var fetchToTimestamp = UnixTimestamp.Min(synchronizationPoint, synchronizationPoint.Add(TimeSpan.FromHours(- _options.MinimumAgeThresholdHours)).RoundToLatestHour());
    ...
    public async Task<List<Measurement>> FetchMeasurements(MeteringPointTimeSeriesSlidingWindow slidingWindow, string meteringPointOwner,
        UnixTimestamp fetchToTimestamp, CancellationToken cancellationToken)
    {
        var fetchFromTimestampSeconds = slidingWindow.GetFetchIntervalStart().Seconds;
        var fetchToTimestampSeconds = fetchToTimestamp.Seconds;

        if (fetchFromTimestampSeconds < fetchToTimestampSeconds)
        {
            var request = new GetMeasurementsRequest
            {
                DateFrom = fetchFromTimestampSeconds,
                DateTo = fetchToTimestampSeconds,
            ...
            _logger.LogInformation(
                ...
                slidingWindow.GSRN,
                DateTimeOffset.FromUnixTimeSeconds(fetchFromTimestampSeconds).ToString("o"),
                DateTimeOffset.FromUnixTimeSeconds(fetchToTimestampSeconds).ToString("o"));
