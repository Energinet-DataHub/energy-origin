...
public class SlidingWindowService
{
    ...
    public MeteringPointTimeSeriesSlidingWindow UpdateSlidingWindow(MeteringPointTimeSeriesSlidingWindow window, List<Measurement> measurements,
        UnixTimestamp newSynchronizationPoint)
    {
        if (NoMeasurementsFetched(measurements))
        {
            ...
            return MeteringPointTimeSeriesSlidingWindow.Create(window.GSRN, newSynchronizationPoint, new List<MeasurementInterval> { interval });
        }

        var missingIntervals = FindMissingIntervals(window, measurements, newSynchronizationPoint);
        return MeteringPointTimeSeriesSlidingWindow.Create(window.GSRN, newSynchronizationPoint, missingIntervals);
    }
