...
public class RabbitMqConnectionTest : IAsyncLifetime
{
    ...
    public async Task GivenMassTransitConfig_WhenConfiguringHost_HealthCanBeConfiguredToCheckRabbitMqConnectivity()
    {
        ...
        await WaitForHealthEndpointResponse(HttpStatusCode.ServiceUnavailable, HttpStatusCode.ServiceUnavailable, HttpStatusCode.ServiceUnavailable);

        // When starting RabbitMQ
        await _rabbitMqContainer.StartAsync();

        // Health check becomes ok
        await WaitForHealthEndpointResponse(HttpStatusCode.OK, HttpStatusCode.OK, HttpStatusCode.OK);

        // When stopping RabbitMQ
        await _rabbitMqContainer.StopAsync();

        // Health check becomes ok
        await WaitForHealthEndpointResponse(HttpStatusCode.OK, HttpStatusCode.ServiceUnavailable, HttpStatusCode.OK);
    }
    ...
    private static WebApplicationBuilder ConfigureWebAppBuilder(DatabaseInfo databaseInfo)
    {
        ...
        builder.Services.AddDbContext<TestDbContext>(
            options => { options.UseNpgsql(databaseInfo.ConnectionString); });
        builder.AddSerilog();
    ...
    private async Task WaitForHealthEndpointResponse(HttpStatusCode expectedLivenessStatusCode, HttpStatusCode expectedReadyStatusCode,
        HttpStatusCode expectedStartupStatusCode)
    {
        ...
        HttpResponseMessage? livenessHealthResponse = null;
        HttpResponseMessage? readyHealthResponse = null;
        HttpResponseMessage? startupHealthResponse = null;

        while (DateTimeOffset.UtcNow < timeout)
        {
            var livenessHealthEndpoint = "health";
            var readyHealthEndpoint = "health/ready";
            var startupHealthEndpoint = "health/startup";

            livenessHealthResponse = await httpClient.GetAsync(livenessHealthEndpoint);
            readyHealthResponse = await httpClient.GetAsync(readyHealthEndpoint);
            startupHealthResponse = await httpClient.GetAsync(startupHealthEndpoint);

            if (livenessHealthResponse.StatusCode == expectedLivenessStatusCode && readyHealthResponse.StatusCode == expectedReadyStatusCode &&
                startupHealthResponse.StatusCode == expectedStartupStatusCode)
            {
                return;
            }
        ...
        throw new Exception(
            $"Health endpoint did not return status code (ready={expectedReadyStatusCode}, liveness={expectedLivenessStatusCode}, startup={expectedStartupStatusCode}) within {timeoutSeconds} seconds, last status code was (ready={readyHealthResponse?.StatusCode}, liveness={livenessHealthResponse?.StatusCode}, startup={startupHealthResponse?.StatusCode})");
    }
