...
To update the database use this command from the Web project folder (replace `CleanTransfer` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../CleanTransfer.Infrastructure/CleanTransfer.Infrastructure.csproj -s CleanTransfer.Web.csproj
```
