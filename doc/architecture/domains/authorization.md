...
The Consent Authorization API endpoints are used for fetching user/client authorization modeinformation. These endpoints are designed to be called from the custom policies running in Azure B2C. The custom policies are the only authorized callers of these endpoint.
